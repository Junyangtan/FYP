...
NUM_RUNS = 10
MAP_NAME = 'Map2'
ALGORITHM_NAME = 'GA_3D_v4'
ANALYSIS_ALGORITHM = 'GA'
OUTPUT_FOLDER = 'Output'
GA_FOLDER = os.path.join(OUTPUT_FOLDER, 'GA_3D')
ROUTES_FOLDER = os.path.join(GA_FOLDER, 'Routes')
METRICS_FOLDER = os.path.join(GA_FOLDER, 'Metrics')
SUMMARY_FOLDER = os.path.join(GA_FOLDER, 'Summary')
BASE_FOLDER = os.path.join(GA_FOLDER, 'Bases')
KML_FOLDER = os.path.join(GA_FOLDER, 'KML')
PARETO_FOLDER = os.path.join(GA_FOLDER, 'Pareto')
...
def make_capacity_safe_clusters():
    data = np.array([[t[0], t[1], t[2] * CLUSTER_ELEVATION_WEIGHT] for t in trees])
...
def decode_route(chromosome, middle_trees, start_tree, end_tree):
    ...
    for gene in chromosome:
        route.append(middle_trees[gene])
...
def _solve_cluster_raw(cluster, start_tree, end_tree):
    ...
    for generation in range(GENERATIONS):
        ...
        for chromosome in population:
            ...
            value = calculate_fitness(route)
            fitness.append(value)
            routes.append(route)
            if value < best_fitness:
                best_fitness = value
                best_route = route.copy()
        elite_indices = sorted(range(len(population)), key=lambda i: fitness[i])[:ELITE_SIZE]
...
def load_map():
    ...
    xy_points = np.array([[t[0], t[1]] for t in trees])
    plantation_boundary = convex_hull(xy_points)
    ...
    with open(ANALYSIS_FILE, 'r') as file:
        ...
        for row in reader:
            if row['Map'] == MAP_NAME and row['Algorithm'] == ANALYSIS_ALGORITHM:
                maximum = max(maximum, int(row['Run']))
    return maximum + 1
...
def solver_objective_slots(cluster_size):
    ...
    return 0 if cluster_size <= 3 else POPULATION_SIZE * GENERATIONS
...
def main():
    ...
    for local_run in range(NUM_RUNS):
        ...
        for cluster_id, cluster in enumerate(ordered_clusters):
            ...
            details.append({'cluster': cluster_id + 1, 'trees': len(cluster), 'route': route, 'distance': distance, 'elevation': elevation, 'turning': turning, 'start_tree': start_tree, 'end_tree': end_tree, 'start_base': cluster_id, 'end_base': (cluster_id + 1) % len(bases)})
        metrics_file = os.path.join(METRICS_FOLDER, f'metrics_{RUN_ID}.csv')
        with open(metrics_file, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Cluster', 'Trees', 'Distance', 'Elevation', 'Turning'])
            for item in details:
                writer.writerow([item['cluster'], item['trees'], item['distance'], item['elevation'], item['turning']])
        ...
        print('GA RUN COMPLETED:', RUN_NUMBER)
