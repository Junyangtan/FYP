...
OUTPUT_FOLDER = os.path.join("Output", "GA_3D")
...
# Original GA population/generation settings.
POP_SIZE = 200
NGEN = 200

# Weighted single-objective GA cost:
#   distance + 10*turning + 2*elevation
DISTANCE_WEIGHT = 1.0
TURN_WEIGHT = 10.0
...
# Start/end palms stay fixed; GA optimizes only the middle palms.
ROW_GAP = 5.0
...
def total_cost(order):
    """
    GA remains a weighted single-objective optimizer.

    The three terms use exactly the same palm-to-palm-only definitions
    that are saved for later algorithm comparison and ANOVA.
    """
    return float(
...
def access_distance_2d(base_xy, tree_id):
    ...
    It is NOT included in GA distance or GA optimization cost.
    """
    tree_xy = np.array(
...
if "FitnessMinGAAnalysis" not in creator.__dict__:
    creator.create(
        "FitnessMinGAAnalysis",
        base.Fitness,
        weights=(-1.0,)
    )

if "IndividualGAAnalysis" not in creator.__dict__:
    creator.create(
        "IndividualGAAnalysis",
        list,
        fitness=creator.FitnessMinGAAnalysis
    )
...
def solve_cluster(
    ...
    GA optimizes the order of all remaining middle trees.

    This preserves the original GA idea:
        minimize distance + 10*turning + 2*elevation

    while using the same route constraints and metric definitions as
    the GA analysis template.
    """
    middle_trees = [
        tree_id
    ...
    def init_individual():
        ...
        return creator.IndividualGAAnalysis(
            genes
        )

    # Re-register per cluster so each chromosome length is handled cleanly.
    for name in [
        "individual",
        "population",
        "evaluate",
    ...
    # Preserve the original GA mutation setting.
    toolbox.register(
        "mutate",
...
for local_run in range(NUM_RUNS):
    ...
    print(
        f"Total GA Cost    : {total_ga_cost:.4f}"
    )
    ...
    print(
        ...
        "in optimized Total Distance or GA Cost.\n"
    )
    ...
    with open(
        ...
        f.write(
            f"Access Distance  : {total_access:.2f} m "
            "(NOT counted in Total Distance or GA Cost)\n\n"
        )
