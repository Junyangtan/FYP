...
def solve_cluster(cluster):
    ...
    toolbox.register("individual", tools.initIterate, creator.IndividualGA, toolbox.indices)
    toolbox.register("population", tools.initRepeat, list, toolbox.individual)

    def evaluate(ind):
        route = convert(ind)
        d = path_distance(route)
        t = turn_penalty(route)
        e = elevation_cost(route)
        return (d + 10*t + 2*e,)
